"use client";

import React, { useState, useEffect } from "react";

interface DrawCounterTablePageProps {
  numberArr: Record<string, number>;
  gameCategory: number;
  loading?: boolean;
}

const DrawCounterTablePage: React.FC<DrawCounterTablePageProps> = ({
  numberArr,
  gameCategory,
  loading = false,
}) => {
  const [counterDraw, setCounterDraw] = useState<
    { number: number; frequency: number }[]
  >([]);

  useEffect(() => {
    if (!loading) {
      const arr: { number: number; frequency: number }[] = [];
      Object.entries(numberArr).forEach(([key, value]) => {
        arr.push({ number: parseInt(key), frequency: value });
      });
      setCounterDraw(arr);
    }
  }, [numberArr, gameCategory, loading]);

  const isRightEdge = (index: number) => (index + 1) % 5 === 0;
  const isBottomEdge = (index: number, total: number, columns = 5) =>
    index < total - columns;

  const renderSkeleton = () => {
    return Array.from({ length: 20 }).map((_, index) => (
      <div
        key={index}
        className={`py-6 px-4 animate-pulse bg-gray-300
        ${!isRightEdge(index) ? "border-r" : ""}
        ${isBottomEdge(index, 20) ? "border-b" : ""}
        border-gray-400`}
      />
    ));
  };

  return (
    <div className="bg-[#0038A8] rounded-xl w-full">
      <div className="grid grid-cols-5 w-full">
        {loading
          ? renderSkeleton()
          : counterDraw.map((num, index) => (
              <div
                key={index}
                className={`text-white relative py-6 px-4 flex flex-col items-center justify-center text-sm
              ${!isRightEdge(index) ? "border-r" : ""}
              ${isBottomEdge(index, counterDraw.length) ? "border-b" : ""}
              border-[#F6BA12]`}
              >
                <div className="absolute top-1 left-1 text-sm text-[#857e59] font-semibold p-1">
                  {num.number}
                </div>
                <div className="text-white font-bold text-3xl">
                  {num.frequency}
                </div>
              </div>
            ))}
      </div>
    </div>
  );
};

export default DrawCounterTablePage;
