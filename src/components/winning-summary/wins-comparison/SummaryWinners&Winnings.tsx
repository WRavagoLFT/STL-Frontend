    ...
    return drawOrders.map((drawOrder) => {
      ...
      return {
        ...
        firstDateWinners: firstDateItems.reduce(
          (sum: number, item: chartOne_Specific) => sum + item.TotalWinners,
          0
        ),
        ...
        firstDateWinnings: firstDateItems.reduce(
          (sum: number, item: chartOne_Specific) =>
            sum + item.TotalPayoutAmount,
          0
        ),
      ...
      gameCategories.forEach((category) => {
        ...
        result[`firstDate${category.replace(/\s+/g, "")}`] =
          firstDateItems.reduce(
            (sum: number, item: any) => sum + item.TotalTumbokPayouts,
            0
          );
        result[`secondDate${category.replace(/\s+/g, "")}`] =
          secondDateItems.reduce(
            (sum: number, item: any) => sum + item.TotalSahodPayouts,
            0
          );
      });
  ...
  const processChart6Data = (
    ...
    return drawOrders.map((drawOrder) => {
      ...
      gameCategories.forEach((category) => {
        ...
        result[`firstDate${category.replace(/\s+/g, "")}`] =
          firstDateItems.reduce(
            (sum: number, item: any) => sum + item.TotalWinners,
            0
          );
        result[`secondDate${category.replace(/\s+/g, "")}`] =
          secondDateItems.reduce(
    ...
    return drawOrders.map((drawOrder) => {
      ...
      return {
        ...
        firstRangeWinners: firstRangeItems.reduce(
          (sum: number, item: chartOne_Range) => sum + item.TotalWinners,
          0
        ),
        ...
        firstRangeWinnings: firstRangeItems.reduce(
          (sum: number, item: chartOne_Range) => sum + item.TotalPayoutAmount,
          0
        ),
